...
const AbsenceCard: React.FC<AbsenceCard> = ({ isAccordianOpen, setIsAccordian, childName, absenceFrom, absenceIn, absenceReason, absenceWhen, id, isAccordianOpenIndex, setIsAccordianIndex}) => {
    const {t} = useTranslation();
    const { localeLang } = useLangugageContext();
    return(
        <div key={id} className="border my-5 border-2 bg-mainGray w-full lg:w-[40rem] sm:w-[1rem] px-5 py-5 rounded-3xl">
                ...
                {isAccordianOpen && (isAccordianOpenIndex === id) && (
